...
#+begin_src jupyter-python
  ...
  for instruction in instructions:
      ...
      indices = [
          i for i in range(len(boxes[destination])) if boxes[destination][i][0] == label
      ]
      if "=" in instruction:
          if indices:
              boxes[destination][indices[0]] = (label, int(f))
          else:
              boxes[destination].append((label, int(f)))
      elif indices:
          boxes[destination].pop(indices[0])


  def score(box):
      return ([x[1] for x in box] * np.arange(1, len(box) + 1)).sum()


  int(sum(np.arange(1, 257) * [score(boxes[i]) for i in range(256)]))
#+end_src
