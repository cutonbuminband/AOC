...
** Part 1
#+begin_src jupyter-python
  base = {0 : 0, 1: 0, 1 + 1j: 0, 2: 1, 2 + 1j: 1 + 1j, 1 + 2j: 1 + 1j, 2+2j: 1 +1j}
  deltas = {key * 1j**k: value * 1j**k
            for key, value in base.items() for k in range(4)}
  ...
  for direction, count in instructions:
      for i in range(int(count)):
          ...
          for idx in range(1, len(rope)):
              rope[idx] += deltas[rope[idx - 1] - rope[idx]]
          seen.append(rope[-1])
  len(set(seen))
