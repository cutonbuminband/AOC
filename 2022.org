...
** Part 1
#+begin_src jupyter-python
translation = {"X": 1, "Y": 2, "Z": 3, "A": 1, "B": 2, "C": 3}
lines = [[translation[x] for x in line.strip().split(" ")] for line in open(datadir / "2.txt").readlines()]
sum([line[1] + 3 * ((line[1] - line[0] + 1) % 3) for line in lines])
#+end_src

** Part 2
#+begin_src jupyter-python
sum([(line[1] + line[0]) % 3 + 1 + 3 * (line[1] - 1) for line in lines])
#+end_src
