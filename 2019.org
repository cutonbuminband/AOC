...
#+begin_src jupyter-python
  ...
  for input_sequence in input_sequences:
      ...
      while current:
          ...
          op = p[ip]
          while op % 100 != 99:
              p, ip, _ = step(p, ip, inputs, outputs)
              op = p[ip]
      if outputs[0] > maxval:
          maxval = outputs[0]
  maxval
...
#+begin_src jupyter-python
  ...
  part1 = program.copy()
  ip = 0
  next(run(part1, inputs=[1]))
#+end_src

** Part 2
#+begin_src jupyter-python
  next(run(program, inputs=[2]))
#+end_src
...
#+begin_src jupyter-python
  from intcode import run
  program = defaultdict(int)
  opcodes = open(datadir / '11.txt').read().split(",")
  for idx, opcode in enumerate(opcodes):
      program[idx] = int(opcode)

  inputs = [0]
  def solve(program, startval, painted=set()):
      program = program.copy()
      position, direction = 0 + 0j, 1j
      field = defaultdict(int)
      count = 0
      inputs = [startval]
      for value in run(program, inputs):
          if count % 2 == 0:
              field[position] = value
              painted.add(position)
          else:
              direction = direction * (1j * (1 - 2 * value))
              position += direction
              inputs.append(field[position])
          count += 1
      return painted, field
  painted, _ = solve(program, 0)
  len(painted)
#+end_src
