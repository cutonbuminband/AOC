...
** Part 1
I really liked this puzzle! The approach I took is to first map out the entire area by giving the droid the necessary instructions, and then using a path finding algorithm to et from start to finish.
#+begin_src jupyter-python
  program = IntCodeProgram(load(15, "np"))
  f = program.run()
  directions = [None, 1j, -1j, -1, 1]



  def navigate(p1, p2, edges):
      visited = set()
      instructions, position = [], p1
      queue = deque([([], p1)])
      while position != p2:
          instructions, position = queue.popleft()
          visited.add(position)
          for neighbor in edges[position]:
              new_position = position + directions[neighbor]
              if new_position in visited:
                  continue
              queue.append((instructions + [neighbor], new_position))
      return instructions
  ...
  while queue:
      ...
      visited.add(position)
      instructions = navigate(old_position, position, edges)
  ...
  len(navigate(0, target, edges))
#+end_src

** Part 2
We mapped out the whole area for part 1, so part 2 is just a flood fill
#+begin_src jupyter-python
  def neighbors(x):
      return [x + directions[direction] for direction in edges[x]]
  utils.bfs(target, None, neighbors)
#+end_src
