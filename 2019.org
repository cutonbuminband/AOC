...
#+begin_src jupyter-python
  ...
  def step(program, ip, inputs=None, outputs=None):
      op = program[ip]
      instruction = op % 100
      mode = [int(x) for x in str(op).rjust(5, '0')[:-2]][::-1]
      if instruction in binops:
          assert mode[2] != 1, "Incorrect mode for destination"
          binargs = [val if mode[idx] == 1 else program[val]
                     for idx, val in enumerate(program[ip + 1: ip + 3])]
          program[program[ip + 3]] = binops[instruction](*binargs)
          ip += 4
      elif instruction == 3:
          if inputs is None:
          ...
          val = program[ip + 1] if mode[0] else program[program[ip + 1]]
          if outputs is None:
              print("Program output: {}".format(val))
          else:
              outputs.append(val)
          ip += 2
      elif instruction == 5 or instruction == 6:
          r = program[ip + 1: ip + 3].copy()
          for idx, value in enumerate(r):
              if not mode[idx]:
                  r[idx] = program[r[idx]]
          if (instruction == 5 and r[0] != 0) or (instruction == 6 and r[0] == 0):
              ip = r[1]
          else:
              ip += 3
      return program, ip
