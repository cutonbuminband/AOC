...
The off instructions can be handled by restricting our sphere of interest to the cube indicated by the off instruction. The effect of the off instruction must be the difference between running the whole sequence but skipping that instruction, and running the whole sequence starting just after the instruction.

We're going to need a way of calculating the intersection of two cuboids. But that's just the intersection of 3 pairs of lines, since the cuboids are axis-aligned. And we can intersect two line segments as follows
#+begin_src jupyter-python
...
From there, we calculate the intersection of two cuboids, and define a helper method to clip an instruction to a cuboid
#+begin_src jupyter-python
  def intersect_cuboids(c1, c2):
      result = [intersect_segments(*pair) for pair in zip(c1, c2)]
      return result if all(result) else False

  def clip_instruction(instruction, cuboid):
      new_region = intersect_cuboids(instruction[1], cuboid)
      return (instruction[0], new_region) if new_region else False
#+end_src

The segments were given as closed intervals, but the parsing converted them to open intervals. The length of each is thus the endpoint minus the starting point. The volume of a cuboid is just the product the three lengths:
#+begin_src jupyter-python
  def cuboid_volume(cuboid):
    return np.product([[line[1] - line[0]] for line in cuboid])
#+end_src

The approach we'll take is to process the list of instructions sequentially, calculating the various intersections as we go. Afterwards, going from intersections to the total score can be accomplished as follows
#+begin_src jupyter-python
  def calculate_score(state):
      result = 0
      sign = 1
      for level in state:
          result += sign * sum(map(cuboid_volume, level))
          sign *= -1
      return result
#+end_src

We also need a way of clipping a state to a cuboid to handle the "off" instructions, and a way of combining two different sets of states
#+begin_src jupyter-python
  def clip_state(state, cuboid):
      state = state.copy()
      result = []
      for level in state:
          new_level = [clipped for c in level if (clipped := intersect_cuboids(c, cuboid))]
          if not new_level:
              break
  ...
  def combine_states(state1, state2):
      result = []
      for l1, l2 in itertools.zip_longest(state1, state2, fillvalue=[]):
          result.append(l1 + l2)
      return result
#+end_src
...
#+begin_src jupyter-python
  def reboot(instructions, state=None):
      if state is None:
          state = []
      if not instructions:
          return calculate_score(state)
      instruction, region = instructions.pop(0)
      if instruction == "on":
          new_state = [[region]] + clip_state(state, region)
          new_state = combine_states(state, new_state)
          return reboot(instructions, new_state)
      if instruction == "off":
          clipped_instructions = [c for i in instructions if (c := clip_instruction(i, region))]
          return (reboot(instructions, state)
                  + reboot(clipped_instructions)
                  - reboot(clipped_instructions, clip_state(state, region)))
  reboot(commands.copy())

#+end_src

In this approach, we tracked a lot of state for the intersections in the reboot function itself, and then had a very simple scoring function. An alternative approach would have been to track only which cuboids were participating in the various combinations, and then calculated all the intersections in the scoring function.
