...
#+begin_src jupyter-python
  ...
  def run(triple, state, w):
      a, b, c = triple
      y, z = state
      if w == z % 26 + b:
          return (0, z // a)
      return (w + c, (z // a) * 26 + w + c)
  ...
  for triple in triples:
      ...
      for prefix, state in states:
          if triple[0] == 26:
              w = state[-1] % 26 + triple[1]
              ws = [w] if 1 <= w < 10 else []
          else:
              ws = range(1, 10)
          new_states += [(prefix + str(w), run(triple, state, w)) for w in ws]
      states = new_states
  max(int(x[0]) for x in states)
#+end_src
** Part 2
After all that, luckily part 2 is trivial
#+begin_src jupyter-python
  min(int(x[0]) for x in states)
